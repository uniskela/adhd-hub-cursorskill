...
From this plugin’s root (or the Marketplace install root / `${CURSOR_PLUGIN_ROOT}` when Cursor documents that path):

```bash
bash skills/env-check/scripts/check_runtime.sh
```

When the skill is resolved from a packaged plugin path, run the same relative script from that plugin install root. Agents may resolve skills from the plugin package path rather than the workspace checkout.
